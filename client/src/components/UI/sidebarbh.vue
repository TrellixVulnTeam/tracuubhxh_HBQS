    ...
    <div class="page-sidebar">
        ...
        <div class="logo-wrapper" style="text-align:center;">
          <router-link to="/"
            ><img
              ...
              src="../../assets/images/avatar/banner_chu.svg"
              alt=""
              style="height:30px; weight:60px;"
          /></router-link>
        </div>
      </div>
      <div class="sidebar custom-scrollbar">
        <div class="sidebar-user text-center">
          ...
          <h6 class="mt-3 f-14">{{currentUser.MA_DONVI}}</h6>
          <p>{{currentUser.TEN_DONVI}}</p>
        </div>
...
export default {
  ...
  computed: {
    ...mapState({
      menuItems: (state) => state.menubh.data,
    }),
    currentUser() {
  ...
  mounted() {
    this.menuItems.filter((items) => {
      if (items.path === this.$route.path)
        this.$store.dispatch("menubh/setActiveRoute", items);
      if (!items.children) return false;
      items.children.filter((subItems) => {
        if (subItems.path === this.$route.path)
          this.$store.dispatch("menubh/setActiveRoute", subItems);
        if (!subItems.children) return false;
        subItems.children.filter((subSubItems) => {
          if (subSubItems.path === this.$route.path)
            this.$store.dispatch("menubh/setActiveRoute", subSubItems);
        });
      });
    });
